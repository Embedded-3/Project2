...
void rt_OneStep(void)
{
   ...
   */
  model1_SetEventsForThisBaseStep(eventFlags);
  ...
  if (need2runFlags[2]) {
    ...
    {
     case 2 :
      currentTime = (extmodeSimulationTime_T) ((model1_M->Timing.clockTick2 * 50)
        + 0)
        ;
      model1_step2();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(model1_M, 0);

  /* Set Final Simulation Time in Ticks */
  errorCode = extmodeSetFinalSimulationTime((extmodeSimulationTime_T) 500000);
  ...
  errorCode = extmodeInit(model1_M->extModeInfo, (extmodeSimulationTime_T *)
    rteiGetPtrTFinalTicks(model1_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(model1_M, true);
    }
  }
