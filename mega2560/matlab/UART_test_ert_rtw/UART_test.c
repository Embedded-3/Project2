...
void UART_test_SetEventsForThisBaseStep(boolean_T *eventFlags)
{
  ...
  eventFlags[2] = ((boolean_T)rtmStepTask(UART_test_M, 2));
}
...
static void rate_monotonic_scheduler(void)
{
  ...
  if (UART_test_M->Timing.TaskCounters.TID[1] == 0) {
    UART_test_M->Timing.RateInteraction.TID1_2 =
      (UART_test_M->Timing.TaskCounters.TID[2] == 0);
  }
  ...
  if ((UART_test_M->Timing.TaskCounters.TID[1]) > 9) {/* Sample time: [0.01s, 0.0s] */
    UART_test_M->Timing.TaskCounters.TID[1] = 0;
  }

  (UART_test_M->Timing.TaskCounters.TID[2])++;
  if ((UART_test_M->Timing.TaskCounters.TID[2]) > 99) {/* Sample time: [0.1s, 0.0s] */
    UART_test_M->Timing.TaskCounters.TID[2] = 0;
  }
}
...
void UART_test_step0(void)             /* Sample time: [0.001s, 0.0s] */
{
...
void UART_test_step1(void)             /* Sample time: [0.01s, 0.0s] */
{
  int16_T i;
  uint8_T data_out[10];
  uint8_T tmp;
  ...
  if (UART_test_B.ready) {
    ...
    UART_test_DW.p_speed_L = (real_T)data_out[2] / 100.0 + (real_T)data_out[1];
    UART_test_DW.p_speed_R = (real_T)data_out[4] / 100.0 + (real_T)data_out[3];
    UART_test_DW.p_v_target = data_out[6];
    UART_test_DW.p_slope = data_out[5];
    tmp = data_out[7];
    if (data_out[7] > 127) {
      tmp = 127U;
    }
  ...
  UART_test_B.crc = UART_test_DW.p_crc;

  /* End of MATLAB Function: '<Root>/Arduino Input2' */
  /* RateTransition generated from: '<Root>/Subsystem' */
  if (UART_test_M->Timing.RateInteraction.TID1_2) {
    UART_test_DW.speed_L_Buffer = UART_test_B.speed_L;

    /* RateTransition generated from: '<Root>/Subsystem' */
    UART_test_DW.speed_R_Buffer = UART_test_B.speed_R;
  }
...
void UART_test_step2(void)             /* Sample time: [0.1s, 0.0s] */
{
  real_T tmp;
  real_T tmp_0;
  real_T tmp_1;
   ...
   *  RateTransition generated from: '<Root>/Subsystem'
   * */
  tmp_1 = floor(UART_test_DW.speed_L_Buffer);
  if (tmp_1 < 256.0) {
    if (tmp_1 >= 0.0) {
  ...
  tmp_0 = floor(UART_test_mod(UART_test_DW.speed_L_Buffer * 100.0));
  if (tmp_0 < 256.0) {
    if (tmp_0 >= 0.0) {
  ...
  tmp_0 = floor(UART_test_DW.speed_R_Buffer);
  if (tmp_0 < 256.0) {
    if (tmp_0 >= 0.0) {
  ...
  tmp = floor(UART_test_mod(UART_test_DW.speed_R_Buffer * 100.0));
  if (tmp < 256.0) {
    if (tmp >= 0.0) {
      frame[3] = (uint8_T)tmp;
    } else {
...
void UART_test_initialize(void)
{
  ...
  UART_test_M->Timing.stepSize0 = 0.001;

  /* External mode info */
  UART_test_M->Sizes.checksums[0] = (817627054U);
  UART_test_M->Sizes.checksums[1] = (2014234330U);
  UART_test_M->Sizes.checksums[2] = (3631626150U);
  UART_test_M->Sizes.checksums[3] = (3319632765U);

  {
    ...
    rteiSetTFinalTicks(UART_test_M->extModeInfo, 300000);
  }
