...
def find_lines(binary_img, left_line, right_line):
    ...
    if len(leftx) > 0:
        ...
        if len(left_line.prevx) > 5:
            avg = smoothing(np.array(left_line.prevx))
            left_fitx = avg
    ...
    if len(rightx) > 0:
        ...
        if len(right_line.prevx) > 5:
            avg = smoothing(np.array(right_line.prevx))
            right_fitx = avg
...
def main():
    ...
    while True:
        ...
        warp_img, M, Minv = warp_image(binary, src, dst, (640, 480))
